...
function Card({ marketData, selectedCoin, correlationData }) {
  ...
  const [cd, setCD] = useState(correlationData);
  ...
  useEffect(() => {
    const regex = /-/;
    const desiredItem = marketData && marketData.filter((item) => {
    ...
    let updatedSpread = getSpread(preferredPrice, baseAmount, desiredPercentage);
    setSpread(updatedSpread);
    // if (priceAdjuster) {
    //   updatedSpread = getSpread(priceAdjuster, baseAmount, desiredPercentage);
    //   setSpread(updatedSpread);
    // } else {
    //   updatedSpread = getSpread(price, baseAmount, desiredPercentage)
    //   setSpread(updatedSpread)
    // }
    
    
    // console.log('Selected Coin: ', desiredItem)
  ...
  return (state && cd && (
    <div className="Card">
      <div className="left-container" style={{ backgroundColor: colorIndicator}}>
      ...
      <header className="main-area">
        ...
        <div className="price-header">
          <h1>{state.name}<img src={state.image.thumb} alt="" /></h1>
          <h1 className="price-indicator">Current Price: ${state.current_price}</h1>
          <h2 className="price-change">Price change in 24hrs: <span style={{color: colorIndicator}}>{state.price_change_24h}</span></h2>
          <h2 className="percent-change">Percent change in 24hrs: <span style={{color: colorIndicator}}>{round(state?.price_change_24h, 2)}</span></h2>
          <h2 className="volume">Total Volume: <span style={{color: volumeColor}}>${numberWithCommas(state?.total_volume)}</span></h2>
        </div>
        <Dial cargo={cargo} />
      </header>
    </div>
